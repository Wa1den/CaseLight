...
{
    const string Api = "https://api.github.com/repos/Wa1den/CaseLight/releases/latest";

    /// <summary>Where to send the user if the answer carried no link of its own.</summary>
    const string Page = "https://github.com/Wa1den/CaseLight/releases/latest";
    ...
    public static async Task<(Version Version, string Url)?> FindNewerAsync(Version current)
    {
        try
        {
            ...
            request.Headers.UserAgent.ParseAdd("CaseLight/" + Three(current));
