...
///
/// Both halves work on the frame before the zones are read off it, so the canvas shows
/// exactly the picture the colours are taken from.
///
...
/// colour it reads is drawn from the neighbourhood around it, so neighbouring lights run
/// into each other.
///
