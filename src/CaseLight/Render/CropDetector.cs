...
/// move in past them instead of averaging the case down with screen that carries nothing.
///
...
///
/// Lives in the application rather than in Core, and so does the Rimlight copy it was taken
/// from: Core is one library kept identical in two repositories, and a file that only the
/// applications call would be one more thing to carry across on every edit.
/// </summary>
...
/// <summary>
/// Moves the sampling zones onto the picture once the bars are known. The layout is left
/// alone - this produces a second set of rectangles to sample through, so the canvas keeps
/// showing where the LEDs actually are.
/// </summary>
